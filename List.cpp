...
const int List::RFile(const std::string fName)
{
	...
	if (this->mDensity == 1)
	{
		...
		for (int iterNodeStart = 0; iterNodeStart < this->mAmOfNodes; iterNodeStart++)
			for (int iterNodeEnd = 0; iterNodeEnd < this->mAmOfNodes; iterNodeEnd++)
				...
				{
					int valueOfConnection = rand() % 50 - regValue;

					while (valueOfConnection == 0)
						valueOfConnection = rand() % 50 - regValue;
		...
		for (int iterEdges = 0; iterEdges < this->mAmOfEdges; iterEdges++)
		{
			...
			{
				int valOfConnection = rand() % 50 - regValue;

				while (valOfConnection == 0)
					valOfConnection = rand() % 50 - regValue;

				if (randStartNode != randEndNode)
					AddEdge(randStartNode, randEndNode, valOfConnection, iterEdges);
